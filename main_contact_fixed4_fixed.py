...
async def user_chapters(update: Update, subject: str, page: int = 0):
    ...
    if isinstance(update, Update):
        ctx_user_data = update.get_bot()._application.user_data  # not used; we use callback path instead
...
async def text_or_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logging.info(f"[contact-debug] text_or_poll called; uid={uid}; pending={pending_contact.get(uid)}; msg={(update.message.text if update.message else None)}")
