...
async def text_or_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "CONTACT_ADMIN" and update.message:
        ...
        try:
            ...
            pass
        context.user_data["mode"] = None
        pending_new_subject.pop(uid, None)
    ...
    if (mode == "NEW_SUBJECT" or pending_new_subject.get(uid)) and update.message and update.message.text:
...
async def btn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Quick handlers for common flows to ensure responsiveness
    if q.data and q.data.startswith("u:subj:"):
        # direct quick dispatch for subject button (user flow)
        subj = q.data.split(":", 2)[2]
        context.user_data["subject"] = subj
        await user_chapters(update, subj, 0)
        return
    if q.data == "a:newsubj":
        # ensure admin 'add subject' prompt is robust
        context.user_data["mode"] = "NEW_SUBJECT"
        pending_new_subject[uid] = True
        await q.message.edit_text("Admin › Add quiz\n\nSend the *Subject* name:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="a:add")]]))
        return
