...
# ------------ Helpers ------------
async def busy(chat, action=ChatAction.TYPING, secs=0.3):  # simple delay to feel responsive
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_chat.send_message(f"Hey {hi}, welcome to the Quiz Bot! 🎓",
...
async def user_chapters(update_or_query, subject, page=0):
    chat = update_or_query.effective_chat if isinstance(update_or_query, Update) else update_or_query.callback_query.message.chat
    await busy(chat)
    chapters = list_chapters_with_counts(subject, ai_only=False)
    if not chapters:
        await edit_or_reply(update_or_query, f"Home › Subjects › {subject}\n\nNo chapters found.",
                            InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="u:startback")]]))
        return
    pages = max(1, ceil(len(chapters) / PAGE_SIZE))
    page = max(0, min(page, pages - 1))
    items = chapters[page*PAGE_SIZE:(page+1)*PAGE_SIZE]
...
async def user_chapters_ai(update_or_query, subject, page=0):
    chat = update_or_query.effective_chat if isinstance(update_or_query, Update) else update_or_query.callback_query.message.chat
    await busy(chat)
    chapters = list_chapters_with_counts(subject, ai_only=True)
    if not chapters:
        await edit_or_reply(update_or_query, f"AI Gen › Subjects › {subject}\n\nNo chapters found.",
                            InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="uai:startback")]]))
        return
    pages = max(1, ceil(len(chapters) / PAGE_SIZE))
    page = max(0, min(page, pages - 1))
    items = chapters[page*PAGE_SIZE:(page+1)*PAGE_SIZE]
...
async def send_next_quiz(bot, session_id: int):
    try:
        ...
        # ✂️ Removed the extra "Controls" message — only /stop works now.
...
async def admin_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---- users & messaging ----
    if act == "users":
        await users_panel(q, page=0); return
    ...
    if act == "admins":
        ...
        pages = max(1, ceil(max(1,len(ids))/PAGE_SIZE))
    ...
    # ---- counters / broadcast / users DB / AI import ----
    if act == "count":
    ...
    # ---- confirmed delete quiz ----
    if act.startswith("delquiz:") or act.startswith("a:delquiz:"):
        qid = int(act.split(":")[-1])
        ...
        if not row:
            ...
            return
        if not (is_owner(uid) or (is_admin(uid) and int(row["added_by"] or 0) == int(uid))):
            await q.message.reply_text("Not permitted."); return
...
async def text_or_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # owner sends message to a specific user
    ...
    if mode == "NEW_SUBJECT" and update.message and update.message.text:
        ...
        context.user_data["mode"] = None
        await update.message.reply_text(f"Subject set to *{context.user_data['add_subject']}*.\nNow tap *Add new Chapter*.",
                                        parse_mode="Markdown")
        return
...
async def btn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("a:"):
        if not is_admin(uid):
            ...
            return
        # special case: top-level import (owner)
        if data == "a:import":
            if not is_owner(uid):
...
# ------------ Keepalive (optional for render/railway) ------------
app = Flask(__name__)
@app.get("/")
def home(): return "OK"
