...
def main_menu(uid: int):
    ...
    # Admins see Admin Panel; only OWNER sees Leaderboard
    if is_admin(uid):
...
async def text_or_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "IMPORT" and update.message and update.message.document:
        ...
        try:
            with open("import.json", "r", encoding="utf-8-sig") as f:  # utf-8 with BOM tolerant
                data = json.load(f)
            count = 0
            for it in data:
                conn.execute(
...
def _guess_subject_chapter(args):
    """Split args into (subject, chapter) by matching known subjects or DB pairs."""
    if not args or len(args) < 2:
        return None, None
    # Try exact DB pair match first
