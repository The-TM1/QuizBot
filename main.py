# main.py — Madhyamik Helper Quiz Bot (Webhook version for Deta Space / any HTTPS host)
# PTB v20.7, Flask webhook endpoint, full feature set from latest polling build.
# Notes:
# - Set env: BOT_TOKEN, ADMIN_ID (optional, comma-separated), DB_PATH (optional),
#            WEBHOOK_SECRET (required for webhook), PUBLIC_URL (required to auto-set webhook)
# - No "typing" action; busy() just sleeps.
# - Webhook endpoint:  POST /webhook/<WEBHOOK_SECRET>
...
ADMIN_IDS_ENV = os.getenv("ADMIN_ID", "")  # comma-separated IDs allowed
DB_PATH = os.getenv("DB_PATH", "db.sqlite3")

WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "")  # required for webhook mode
PUBLIC_URL = os.getenv("PUBLIC_URL", "")          # e.g. https://<your-app>.<user>.deta.app
...
async def busy(chat, action=ChatAction.TYPING, secs=0.25):
    # No typing action (to reduce noise); tiny sleep for debounce
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Use /stop any time to cancel. Admins can /delsub, /delchap, /delquiz and edit."
    )
...
async def admin_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if act == "panel":
        ...
        await busy(q.message.chat, ChatAction.UPLOAD_DOCUMENT, secs=0.15)
...
async def text_or_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "CONTACT_ADMIN" and update.message:
        u = update.effective_user
        header = f"📨 Message to admin from @{u.username or u.first_name} (id:{u.id}):"
        for aid in admin_ids_from_settings():
            try:
                await context.bot.send_message(aid, header)
                await context.bot.copy_message(chat_id=aid,
...
# PTB application & loop globals for webhook processing
application = None
bot_loop = None
...
if __name__ == "__main__":
    ...
    if not WEBHOOK_SECRET:
        raise SystemExit("WEBHOOK_SECRET missing in environment.")
    if not PUBLIC_URL:
        raise SystemExit("PUBLIC_URL missing in environment (e.g. https://<app>.<user>.deta.app)")
    ...
    # Start PTB in its own event loop thread
    def start_bot_loop():
        global bot_loop
        ...
        # Set Telegram webhook to our HTTPS URL
        url = f"{PUBLIC_URL}/webhook/{WEBHOOK_SECRET}"
        bot_loop.run_until_complete(application.bot.set_webhook(url=url, allowed_updates=Update.ALL_TYPES))
