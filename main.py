...
# New: Admin management (/admins, /addadmin <id>, /rmadmin <id>)
...
load_dotenv("secrets.env")  # safe: only loads if file exists locally
...
async def busy(chat, action=ChatAction.TYPING, secs=0.25):
    await asyncio.sleep(secs)  # no typing action to reduce noise
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Use /stop any time to cancel. Admins can manage quizzes and broadcast."
    )
...
if __name__ == "__main__":
    ...
    # POLLING mode
    # Ensure webhook is off (PTB will call deleteWebhook internally on start())
